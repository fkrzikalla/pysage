...
def convert_dataframe_to_canonical( df:pd.DataFrame, units:dict )->pd.DataFrame:
    ...
    for log_name in numeric_columns:
        ...
        if log_units in units_table:
            ...
            log_values = raw_data[log_name].to_numpy()
            new_log, new_symbol = convert_to_canonical( log_values, log_units) 
            new_log, new_symbol

            #update the data 
            raw_data[log_name] = new_log
            units[log_name] = new_symbol
        else:
            print( '{} was not converted because the unit {} is not known '.format(log_name,log_units))
    
    
def convert_to_system( system_name, measurement:Union[np.ndarray,int,float], symbol:str )->Tuple[ Union[np.ndarray,int,float],str]:
...
def convert_to_english( measurement:Union[np.ndarray,int,float], symbol:str )->Tuple[ Union[np.ndarray,int,float],str]:
